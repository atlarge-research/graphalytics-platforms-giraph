...
public class DirectedLocalClusteringCoefficientComputation extends
	...
	public void compute(Vertex<LongWritable, DoubleWritable, NullWritable> vertex,
		...
		if (getSuperstep() == 0) {
			...
			msgObject.setSource(vertex.getId().get());
			...
			collectNeighbourSet(vertex, messages);
			sendConnectionInquiries(vertex.getId().get());
			vertex.getValue().set(neighbours.size());
	...
	private void collectNeighbourSet(Vertex<LongWritable, DoubleWritable, NullWritable> vertex,
	...
	private static double computeLCC(double numberOfNeighbours, Iterable<LocalClusteringCoefficientMessage> messages) {
		...
		return numberOfMatches / numberOfNeighbours / (numberOfNeighbours - 1);
	}
